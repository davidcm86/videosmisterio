...
class VideosController extends AppController
{
    ...
    {
        parent::initialize();
        $this->Auth->allow(['misterio', 'sitemap', 'usuario', 'seoPosicionGoogle', 'rellenarDescripcionWikipedia']);
    ...
    public function misterio($slugVideo = null)
    {
        ...
        if (count($videosRandom) < 12) {
            ...
            $videosCategoriaRandom = $this->Videos->find('all', [
                ...
                'order' => 'rand()', 
                'fields' => ['Videos.titulo', 'Videos.descripcion', 'Videos.created', 'Videos.enlace', 'Videos.id', 'Videos.slug_titulo', 'Videos.keywords'],
                'limit' => $limitRellenar
            ])->toArray();
    ...
    public function subeTuVideoDeMisterio()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $textoWiki = $this->__getTextFromWikipedia(strtolower(Inflector::slug($this->request->data['titulo'],'+')));
            if (!empty($textoWiki)) {
                $subirVideo->descripcion = $this->request->data['descripcion'] . ' ' . $textoWiki;
            }
    ...
    /*public function rellenarDescripcionWikipedia($fechaDesde = "2017-09-30", $pass = 'sdfsdf34rfdsf') {
        echo "Inicio";
        set_time_limit(0);
        $this->autoRender = false;
        if (empty($fechaDesde) || empty($pass)) die;
        $videos = $this->Videos->find('all', [
            'conditions' => ['created >= ' => $fechaDesde]
        ])->toArray();
        foreach ($videos as $video) {
            $textoWiki = $this->__getTextFromWikipedia(strtolower(Inflector::slug($video->keywords,'+')));
            var_dump($textoWiki);
            if (!empty($textoWiki)) {
                $videoGet = $this->Videos->get($video->id, [
                    'contain' => []
                ]);
                $videoGet->descripcion = $video->descripcion . ' ' . $textoWiki;
                if (!$this->Videos->save($videoGet)) die('Error salvando wiki');
                sleep(3);
            }
        }
        echo "Fin";
